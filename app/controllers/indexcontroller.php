...
class IndexController extends AbstractController
{
    ...
    public function DefaultAction()
    {
        ...
        if (Request::Check('submit-checkin')) {
            ...
            if (!empty($recaptcha)) {
                $response = $recaptcha_object->verifyResponse($recaptcha);
                if (isset($response['success']) and $response['success'] === true) {
                    ...
                    $repair->devicePassword = Cipher::Instance()->Encrypt(Request::Post('device-password', false, true));
                    $repair->deviceAltPassword = Cipher::Instance()->Encrypt(Request::Post('device-password-alt', false, true));
                    ...
                    if ($repair->Save()) {
                        ...
                        if ($repair->emailUpdates == 1) {
                            try {
                                ...
                                if ($mail->Send()) {
                                    $this->logger->info("Job Notification Email was sent to customer $customer_details->lastName Regarding Job", array('job_id' => $repair->job_id));
                                } else {
                                    throw new \Exception();
                                }
                            } catch (\Exception $e) {
...
//                }
                    ...
                    Session::Set('job-checkin', "Error, Failed to verify Captcha.");
                }
            } else {
                Session::Set('job-checkin', "Error, Captcha is required.");
                $this->session->set_flashdata('error', "Captcha is required.");
            }
        }
        ...
        $this->_template
            ->SetViews(['view'])
            ->SetData([
                'script' => $recaptcha_object->getScriptTag(),
                'widget' => $recaptcha_object->getWidget()
            ])
            ->Render();
    }
